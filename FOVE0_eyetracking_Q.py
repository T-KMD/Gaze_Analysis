...
def checkAnswer(x,y,list1,list2):
    ...
    for i in range(0,6):
        if i == 1 or i == 5:
            continue
        if list1[i][0] < x < list1[i][1] and list2[i][0] < y < list2[i][1]:
            answerflag = i + 1
...
def fove0_eyetracking():
    ...
    with open(sys.argv[3], 'r', newline='') as csvfile:
        target_range_file_presenter = open(sys.argv[4],'r')
        target_range_file_teacher = open(sys.argv[5],'r')

        csv_reader = csv.reader(csvfile, delimiter=',')
        range_reader_presenter = csv.reader(target_range_file_presenter,delimiter=',')
        range_reader_teacher = csv.reader(target_range_file_teacher,delimiter=',')
        ...
        while True:
            ...
            target_range_presenter = next(range_reader_presenter)

            presenter_range_x_min = int(float(target_range_presenter[0]))
            presenter_range_x_max = int(float(target_range_presenter[1]))
            
            presenter_range_y_min = int(float(target_range_presenter[2]))
            presenter_range_y_max = int(float(target_range_presenter[3]))


            target_range_teacher = next(range_reader_teacher)

            teacher_range_x_min = int(float(target_range_teacher[0]))
            teacher_range_x_max = int(float(target_range_teacher[1]))
            
            teacher_range_y_min = int(float(target_range_teacher[2]))
            teacher_range_y_max = int(float(target_range_teacher[3]))
            ...
            if (First_flag == 0 and dX_L < 900) or (dX_L < 900 and (deg_L < 1.5 and deg_R < 1.5 )):
                ...
                LeftEyeFlag = checkAnswer2(LeftEyeFlag,2,x_L,y_L, presenter_range_x_min, presenter_range_x_max, presenter_range_y_min, presenter_range_y_max)
                LeftEyeFlag = checkAnswer2(LeftEyeFlag,6,x_L,y_L,teacher_range_x_min,teacher_range_x_max,teacher_range_y_min,teacher_range_y_max)
                ...
                RightEyeFlag = checkAnswer2(RightEyeFlag,2,x_R,y_R, presenter_range_x_min, presenter_range_x_max, presenter_range_y_min, presenter_range_y_max)
                RightEyeFlag = checkAnswer2(RightEyeFlag,6,x_R,y_R,teacher_range_x_min,teacher_range_x_max,teacher_range_y_min,teacher_range_y_max)
